...
process.stdin.on("data", (data) => {
  const workerData = JSON.parse(data.toString());
  ...
  for (let i = 0; i < workerData.indexes.length; i++) {
    const index = workerData.indexes[i];
    const theta = Examples.row(1).map(zero);
    thetas.push(
      descendGradient(theta, Examples, Classifications.column(index))
    );
  }

  process.stdout.write(
    JSON.stringify({
      startNumber: workerData.indexes[0],
      thetas,
    })
  );
});
